...
function material_news_set($data, $attach_id,$cats=null)
{
    global $_W;
...
function material_news_list($server = '', $search = '', $page = array('page_index' => 1, 'page_size' => 24), $cats=null)
{
    global $_W;
    ...
    if (empty($search)) {
        ...
        if (!empty($cats) && in_array($cats, array('hfgh', 'czgl', 'hwzc'))) {
            $news_list = $wechat_attachment_table->searchWithCats($cats)->orderby('createtime DESC')->getall();
        }else{
        ...
        if (!empty($cats) && in_array($cats, array('hfgh', 'czgl','hwzc'))){
            $wechat_news_table->searchWithCats($cats);
        }
...
{
    $result = material_local_upload($attach_id);
    if (is_error($result)) {
        return $result;
    }
    $tag = $result['url'];
...
{
    $local = material_network_to_local($url, $uniacid, $uid, $type);
    if (is_error($local)) {
        return $local;
    }
