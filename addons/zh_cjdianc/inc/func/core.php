...
class Core extends WeModuleSite {
	...
	public function getMainMenu() {
		...
		if ($_W['role'] == 'operator') {
			...
			if ($system['hygn'] == 1) {
				$navemenu[16] = array(
					...
					'items' => array(
						...
						1 => $this->createMainMenu('开卡记录', $do, 'hyorder', ''),
						2 => $this->createMainMenu('会员期限', $do, 'hyqx', ''),
			...
			if ($system['is_sss'] == 1) {
				array_push($navarr, $this->createMainMenu('闪时送 ', $do, 'sss', ''));
			}
			$navemenu[14] = array(
...
//开启单独手续费
	...
	public function getKeyword($id) {
		global $_GPC, $_W;
		$url = 'https://api.weixin.qq.com/wxaapi/newtmpl/getpubtemplatekeywords?access_token=' . $this->getaccess_token() . '&tid=' . $id;
		$data = $this->httpRequest($url);
		return json_decode($data, true)['data'];
	}
	//生成模板
	public function generateTemplate($id, $keyword_id_list, $note) {
		global $_GPC, $_W;
		...
		$url = 'https://api.weixin.qq.com/wxaapi/newtmpl/addtemplate?access_token=' . $this->getaccess_token();
		$formWork = [
			'tid' => $id,
			'kidList' => $list,
			'sceneDesc' => $note,
