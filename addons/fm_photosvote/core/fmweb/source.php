...
if ($op == 'display') {
	if (!empty($_GPC['displayorder'])) {
		...
		$this->message('来源排序更新成功！', 'refresh', 'success');
	}
	$children = array();
	...
	if (checksubmit('submit')) {
		...
		$this->message('更新来源成功！', $this->createWebUrl('source',array('rid'=>$rid)), 'success');
	}
	include $this->template('web/source');
} elseif ($op == 'source') {
	load()->func('tpl');
	...
	if (empty($category)) {
		$this->message('抱歉，来源不存在或是已经被删除！',  $this->createWebUrl('source',array('rid'=>$rid)), 'error');
	}

	pdo_delete($this->table_source, array('id' => $id, 'parentid' => $id), 'OR');
	$this->message('来源删除成功！',  $this->createWebUrl('source',array('rid'=>$rid)), 'success');
} elseif ($op == 'deleteall') {
