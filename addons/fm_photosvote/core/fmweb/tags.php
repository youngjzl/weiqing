...
if ($op == 'display') {
	if (!empty($_GPC['displayorder'])) {
		...
		$this->message('分组排序更新成功！', 'refresh', 'success');
	}
	$children = array();
	...
	if (!empty($parentid)) {
		...
		if (empty($parent)) {
			$this->message('抱歉，上级分组不存在或是已经被删除！', $this->createWebUrl('tags',array('rid'=>$rid)), 'error');
		}
	}

	if (checksubmit('submit')) {
		if (empty($_GPC['title'])) {
			$this->message('抱歉，请输入分组名称！');
		}
		$data = array(
			'uniacid' => $_W['uniacid'],
			'rid' => $rid,
		...
		$this->message('更新分组成功！', $this->createWebUrl('tags',array('rid'=>$rid)), 'success');
	}
	include $this->template('web/tags');
} elseif ($op == 'tags') {
	load()->func('tpl');
	...
	$this->message($category, '', 'ajax');
} elseif ($op == 'delete') {
	$id = intval($_GPC['id']);
	$category = pdo_fetch("SELECT id, parentid FROM ".tablename($this->table_tags)." WHERE id = '$id'");
	if (empty($category)) {
		$this->message('抱歉，分组不存在或是已经被删除！',  $this->createWebUrl('tags',array('rid'=>$rid)), 'error');
	}

	pdo_delete($this->table_tags, array('id' => $id, 'parentid' => $id), 'OR');
	$this->message('分组删除成功！',  $this->createWebUrl('tags',array('rid'=>$rid)), 'success');
} elseif ($op == 'deleteall') {
