...
$afrom_user = $_GPC['afrom_user'];
	...
	$status = '9';
	$list = pdo_fetchall("SELECT * FROM ".tablename($this->table_bbsreply)." WHERE rid = :rid AND status = :status  ".$uni." ", array( ':rid' => $rid, ':status' => $status));



		include $this->template('web/addmessage');
