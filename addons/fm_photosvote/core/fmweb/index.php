...
defined('IN_IA') or exit('Access Denied');
		...
		$psize = 10;
		$fm_list = pdo_fetchall('SELECT * FROM '.tablename($this->table_reply).' WHERE find_in_set('.$uniacid.', binduniacid) OR uniacid = '.$uniacid.' order by `isadmin` desc, `id` desc LIMIT ' . ($pindex - 1) * $psize . ',' . $psize);
		...
		if (!empty($fm_list)) {
			foreach ($fm_list as $mid => $list) {
				...
				//print_r($money);
			        ...
			        $fm_list[$mid]['user_cyrs'] = $rdisplay['cyrs_total']+$rdisplay['xuninum']+$rdisplay['hits'];//参与人数
