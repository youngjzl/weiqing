...
if ($op == 'display') {
	$this->recoverps($rid, $_GPC['tfrom_user']);
	...
	if (!empty($keyword)) {
		...
		$where .= " OR mobile_info LIKE '%{$keyword}%'";
	...
	if (!empty($_GPC['isdel'])) {
		if ($_GPC['isdel'] == 3) {
			$where .= "AND nickname = '' AND avatar = ''";
		}elseif ($_GPC['isdel'] == 2) {
			$where .= "AND mobile_info = ''";
		}else{
	...
	if(checksubmit('export_submit', true)) {
		$sql = "SELECT * FROM". tablename($this -> table_log) . " WHERE rid = :rid " . $where;
		$users = pdo_fetchall($sql, $params);
		$html = $this->fm_votes_download($users);
		$time = date('Y-m-d H:i:s', time());

		$filename =$k.'投票记录情况'.'_'.$rid.'_'.$time;
	...
	$this->recoverps($rid, '', 'all');
	$this->message('success', $this->createWebUrl('votelog', array('rid' => $rid)));
}
