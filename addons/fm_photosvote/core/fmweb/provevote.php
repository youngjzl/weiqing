	...
	if (!empty($rid)) {
		...
		if ($foo == 'display') {
			...
			if ($_GPC['sh'] == 1) {
				...
				$this->message('审核通过成功！',referer(),'success');
			}
			if ($_GPC['sh'] == 'all') {
				$users_o = pdo_fetchall('SELECT from_user FROM '.tablename($this->table_users).' WHERE rid= :rid AND status != 1' , array(':rid' => $rid) );
				foreach ($users_o as $key => $row) {
					$from_user = $row['from_user'];
					pdo_update($this->table_users, array('status' => '1', 'lasttime' => $now), array('from_user' => $from_user, 'rid' => $rid));
					$this->sendMobileHsMsg($from_user, $rid, $uniacid);
				}
				$this->message('全部审核通过成功！',referer(),'success');
			}
			...
			if (empty($_GPC['time']['start'])) {
				$starttime =strtotime(date("Y-m-d H:i", $now - 2592000));
				$endtime = strtotime(date("Y-m-d H:i", $now + 86400));
			}else{
			...
			$videoarr = pdo_fetchall("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid and status = :status ORDER BY id ASC", array(':from_user' => $from_user,':rid' => $rid,':status' => 1));//显示所有图片
			...
			if (checksubmit('submit')) {
				if ($item['uid']) {
					if ($item['uid'] != $_GPC['uid']) {
						...
						if(!empty($yuid)) {
							$this->message('非常抱歉，此ID已经存在，你需要更换其他ID！');
				...
				if($reply['ismobile']){
					if(!preg_match(REGULAR_MOBILE, $_GPC['mobile'])) {
						$this->message('必须输入手机号，格式为 11 位数字。');
					}
					if ($item['mobile']) {
						if ($item['mobile'] != $_GPC['mobile']) {
							...
							if(!empty($ymobile)) {
								$this->message('非常抱歉，此手机号码已经被注册，你需要更换注册手机号！');
							}
						}
					}else{
						$ymobile = pdo_fetch("SELECT * FROM ".tablename($this->table_users)." WHERE mobile = :mobile and rid = :rid", array(':mobile' => $_GPC['mobile'],':rid' => $rid));
				...
				if (empty($item)) {
					...
					$this->message('成功添加一位参赛选手', referer(), 'success');
				}else{
					...
					if ($_GPC['member'] == '1') {
						$this->message('报名用户更新成功！', $this->createWebUrl('members', array('rid' => $rid, 'foo' => 'display')), 'success');
					}else {
						$this->message('报名用户更新成功！', $this->createWebUrl('provevote', array('rid' => $rid, 'foo' => 'display')), 'success');
					}
				}
