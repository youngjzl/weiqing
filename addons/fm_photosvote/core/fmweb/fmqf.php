...
if($operation == 'display') {
	...
	if (!empty($id)) {
		...
		$ccate = $item['ccate'];
		if (empty($item)) {
			$this->message('抱歉，文章不存在或是已经删除！', '', 'error');
		}
	...
	if (checksubmit('submit')) {
		...
		$this->message('文章更新成功！', $this->createWebUrl('fmqf', array('op' => 'display', 'rid' => $rid)), 'success');
	} else {
		include $this->template('web/fmqf');
	}
	...
	$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename($this->table_qunfa) . " WHERE uniacid = '{$_W['uniacid']}' AND rid = '{$rid}'  $condition");
	$pager = pagination($total, $pindex, $psize);
	...
	include $this->template('web/fmqf');
	 ...
	 global $_GPC, $_W;
		...
		$uniacid = $_W['uniacid'];
		if(!$id){
		    $this->message('文章不存在', $this->createWebUrl('fmqf', array('op' => 'display', 'rid' => $rid)), 'error');
		} else {
			if ($id != 'end') {
				$item = pdo_fetch("SELECT * FROM ".tablename('site_article')." WHERE id = :id" , array(':id' => $id));
