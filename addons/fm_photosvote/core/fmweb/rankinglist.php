...
if ($op == 'tags') {
	...
	$psize = 15;
	$order = '';
	$list_praise = pdo_fetchall("SELECT id, title,piaoshu,parentid FROM ".tablename($this->table_tags)." WHERE rid = :rid  ". $where . $uni ." ORDER BY id DESC  LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':rid' => $rid));
