...
if ($op == 'display') {
	if (!empty($_GPC['displayorder'])) {
		...
		$this->message('学校排序更新成功！', 'refresh', 'success');
	}
	$children = array();
	...
	if (checksubmit('submit')) {
		...
		$this->message('更新学校成功！', $this->createWebUrl('school',array('rid'=>$rid)), 'success');
	}
	include $this->template('web/school');
} elseif ($op == 'school') {
	load()->func('tpl');
	...
	if (empty($category)) {
		$this->message('抱歉，学校不存在或是已经被删除！',  $this->createWebUrl('school',array('rid'=>$rid)), 'error');
	}

	pdo_delete($this->table_school, array('id' => $id, 'parentid' => $id), 'OR');
	$this->message('学校删除成功！',  $this->createWebUrl('school',array('rid'=>$rid)), 'success');
} elseif ($op == 'deleteall') {
