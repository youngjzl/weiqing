...
$op = in_array($op, array('rbasic', 'rdisplay', 'rvote', 'rhuihua', 'rshare', 'rbody', 'rcustom', 'rupload', 'rjifen', 'raward', 'rstar')) ? $op : 'rbasic';
...
if ($op == 'rbasic') {
	...
	$rowrule = pdo_fetchall("SELECT * FROM ".tablename('rule_keyword')." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
	$now = time();
	$reply['title'] = empty($reply['title']) ? "女神来了!" : $reply['title'];
	$reply['status'] = empty($reply['status']) ? '0' : $reply['status'];
	...
	$reply['iparr'] = empty($reply['iparr']) ? $ip['region'] : $reply['iparr'];

	if ($_GPC['keydel']) {
		$key = pdo_fetch("SELECT * FROM ".tablename('rule_keyword')." WHERE id = :id ORDER BY `id` DESC", array(':id' => $_GPC['id']));
		if (empty($key)) {
			$msg = '此关键字不存在！';
			$fmdata = array(
				"success" => -1,
		...
		pdo_delete('rule_keyword', array('id'=>$_GPC['id']));
		$msg = '删除成功！';
		$fmdata = array(
			"success" => 1,
			"msg" => $msg,
	...
	if (checksubmit('submit')) {
		...
		$insert_basic['iparr'] = empty($_GPC['iparr']) ? $ip['region'] : $_GPC['iparr'];

		$qiniu = array(
			'isqiniu' => 0
		);
		if (empty($reply['qiniu'])) {
			$insert_basic['qiniu'] = iserializer($qiniu);
		}

		$insert_basic['fmset'] = empty($_GPC['set']) ? '' : $_GPC['set'];
		if (empty($reply)) {
		...
		if (!empty($_GPC['set'])) {
			$this->message('保存成功，正在进入下一步: 显示设置', $this->createWebUrl('system', array('op'=>$_GPC['set'], 'rid' => $rid, 'set' => 'rvote')), 'success');
		}else{
	...
	$rdisplay['open_vote_count'] = !isset($rdisplay['open_vote_count']) ? "1" : $rdisplay['open_vote_count'];
	...
	if (checksubmit('submit')) {
		...
		pdo_update($this->table_reply, array('fmset' => empty($_GPC['set']) ? '' : $_GPC['set']), array('rid' => $rid));
		...
		if (!empty($_GPC['set'])) {
			$this->message('保存成功，正在进入下一步: 投票设置', $this->createWebUrl('system', array('op'=>$_GPC['set'], 'rid' => $rid, 'set' => 'rhuihua')), 'success');
		}else{
			$this->message('更新成功！', referer(), 'success');
		}
	}
	...
	$rvote['tpxz_video'] = empty($rvote['tpxz_video']) ? "1" : $rvote['tpxz_video'];
	...
	$rvote['isallow_modify'] = !isset($rvote['isallow_modify']) ? "1" : $rvote['isallow_modify'];
	...
	if (checksubmit('submit')) {
		...
		$insert_vote = array(
			...
			'isgetiparr' => $_GPC['isgetiparr'] == 'on' ? 1 : 0,
			...
			'tpxz' => $_GPC['tpxz'] > 30 ? '30' : intval($_GPC['tpxz']),
			'tpxz_video' => $_GPC['tpxz_video'] > 30 ? '30' : intval($_GPC['tpxz_video']),
			...
			'isallow_modify' => $_GPC['isallow_modify'] == 'on' ? 1 : 0,
			'isopenjsps' => $_GPC['isopenjsps'] == 'on' ? 1 : 0,
		);
		...
		pdo_update($this->table_reply, array('binduniacid' => $gduni.','.$binduniacid, 'fmset' => empty($_GPC['set']) ? '' : $_GPC['set']), array('rid' => $rid));

		if (!empty($_GPC['set'])) {
			$this->message('保存成功，正在进入下一步: 会话设置', $this->createWebUrl('system', array('op'=>$_GPC['set'], 'rid' => $rid, 'set' => 'rshare')), 'success');
		}else{
			$this->message('更新成功！', referer(), 'success');
		}
	}
	...
	if (checksubmit('submit')) {
		...
		pdo_update($this->table_reply, array('fmset' => empty($_GPC['set']) ? '' : $_GPC['set']), array('rid' => $rid));
		if (!empty($_GPC['set'])) {
			$this->message('保存成功，正在进入下一步: 关注分享', $this->createWebUrl('system', array('op'=>$_GPC['set'], 'rid' => $rid, 'set' => 'rbody')), 'success');
		}else{
			$this->message('更新成功！', referer(), 'success');
		}
	}
	...
	if (checksubmit('submit')) {
		...
		pdo_update($this->table_reply, array('fmset' => empty($_GPC['set']) ? '' : $_GPC['set']), array('rid' => $rid));
		if (!empty($_GPC['set'])) {
			$this->message('保存成功，正在进入下一步: 页面设置', $this->createWebUrl('system', array('op'=>$_GPC['set'], 'rid' => $rid, 'set' => 'finish')), 'success');
		}else{
			$this->message('更新成功！', referer(), 'success');
		}
	}
	...
	if (checksubmit('submit')) {
		...
		pdo_update($this->table_reply, array('fmset' => empty($_GPC['set']) ? '' : $_GPC['set']), array('rid' => $rid));
		if ($_GPC['set'] == 'finish') {
			$this->message('恭喜，您的活动已经创建成功！ ', $this->createWebUrl('members', array('rid' => $rid)), 'success');
		}else{
			$this->message('更新成功！', referer(), 'success');
		}
	}
...
/*	$foo = empty($_GPC['foo']) ? '' : $_GPC['foo'] ;
	...
	$rjifen = pdo_fetch("SELECT * FROM ".tablename($this->table_jifen)." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
	$rjifen_gift = pdo_fetchall("SELECT * FROM ".tablename($this->table_award)." WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
	load()->func('tpl');
	...
	if (checksubmit('submit')) {
		...
		if (!empty($rjifen['rid'])) {
			pdo_update($this->table_award, $insert_award, array('rid' => $rid));
		} else {
			pdo_insert($this->table_award, $insert_award);
		}
		$this->message('更新成功！', referer(), 'success');
	}
} elseif ($op == 'rstar') {
