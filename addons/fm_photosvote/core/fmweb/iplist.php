...
$reply = pdo_fetch('SELECT * FROM '.tablename($this->table_reply).' WHERE rid =:rid ', array(':rid' => $rid) );
		...
		if ($foo == 'display') {
				...
				$vote = pdo_fetchall("SELECT distinct(ip) FROM ".tablename($this->table_log)." WHERE rid = :rid  ".$uni."  ", array(':rid' => $rid));
				$tvtotal = array();
				foreach ($vote as $v) {
					$total = pdo_fetchcolumn('SELECT COUNT(1) FROM '.tablename($this->table_log).' WHERE rid= :rid AND ip = :ip '.$uni.'  order by `ip` desc ', array(':rid' => $rid, ':ip' => $v['ip']));
					$tvtotal[$v[ip]] .= $total;
			...
			//取得ip详细数据
			$iplist = pdo_fetchall('SELECT * FROM '.tablename($this->table_iplist).' WHERE rid= :rid '.$uni.' order by `createtime` desc LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, array(':rid' => $rid));
			//$iparr = iunserializer($item['iparr']);


			$total = pdo_fetchcolumn('SELECT COUNT(1) FROM '.tablename($this->table_iplist).' WHERE rid= :rid '.$uni.' order by `createtime` desc ', array(':rid' => $rid));
		...
		include $this->template('web/iplist');
