...
if ($op == 'display') {
    ...
    if (is_array($ret) && $ret['result'] == 1) {
      $templatefiles = "";
      $ft = "";
        if ($ret['result'] == 1) {
            $files = array();
            if (!empty($ret['files'])) {
            		if ($version >= '3.991' || $version == '1.0') {
            			if ($ret['fplus']) {
            				$fmpath = IA_ROOT . "/";
					}else{
						$fmpath = IA_ROOT . "/addons/".$_GPC['m']."/";
                ...
                foreach ($ret['files'] as $file) {
                		/**$m = strstr($file['path'],$_GPC['m']);
                		$f = strstr($file['path'],'fm_fplus');
    ...
    if (!empty($path)) {
        ...
        $geetestlib = IA_ROOT . "/addons/fm_photosvote/core/plugin/jiyan/lib/geetestlib.php";
        ...
        if (is_file($geetestlib)) {@unlink($geetestlib);}
