...
if($operation == 'display') {
	...
	if (checksubmit('submit')) {
		...
		if(!preg_match(REGULAR_STYLENAME, $_GPC['stylename'])) {
			$this->message('必须输入模板标识，格式为 字母（不区分大小写）+ 数字,不能出现中文、中文字符');
		}
		if (empty($_GPC['title'])) {
			$this->message('标题不能为空，请输入标题！');
		}
		$data = array(
			'uniacid' => $_W['uniacid'],
			'title' => $_GPC['title'],
		...
		$this->message('模板更新成功！', $this->createWebUrl('templates', array('op' => 'display', 'rid' => $rid)), 'success');
	}
	include $this->template('web/templates');
	...
	//}
	$pageid = !empty($datas['id']) ? $datas['id'] : $_GPC['pageid'];
	...
	$this->message('删除成功！', $this->createWebUrl('templates', array('op' => 'display', 'rid' => $rid)), 'success');
	...
	if ($_W['ispost']) {
		if ($apido == 'savepage') {
			...
			if (empty($id)) {
				...
				$row = pdo_fetch("SELECT id FROM ".tablename($this->table_designer)." WHERE stylename = :stylename AND pagetype = :pagetype", array(':stylename' => $stylename,':pagetype' => $pagetype));

				if (empty($row)) {
					$insert['createtime'] = $date;
					pdo_insert($this->table_designer, $insert);
				}else{
					pdo_update($this->table_designer, $insert, array('stylename' => $stylename,'pagetype' => $pagetype));
				}
			...
			if (empty($pageid)) {
				$this->message('删除失败！Url参数错误', $this->createWebUrl('templates'), 'error');
			} else {
