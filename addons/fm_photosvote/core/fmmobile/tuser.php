...
$videoarr = pdo_fetchall("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid and status = :status ORDER BY id ASC", array(':from_user' => $tfrom_user,':rid' => $rid,':status' => 1));//显示所有视频
