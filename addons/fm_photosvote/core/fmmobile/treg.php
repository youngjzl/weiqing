		...
		if ($_GPC['treg'] == 1) {
			...
			if ($rvote['mediatypev']) {
				if ($rvote['ismediatypev']) {
					$photosvideo = pdo_fetch("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid", array(':from_user' => $from_user,':rid' => $rid));

					if (empty($mygift['vedio']) && empty($_GPC['youkuurl']) && empty($photosvideo['videopath'])) {
						$msg = '请上传视频或者填写网络视频地址！';
						$fmdata = array(
							"success" => -1,
							"msg" => $msg,
		    ...
		    $now = time();
				...
				if ($mygift['weid'] != '200') {
					if ($_W['account']['level'] == 4){
						...
						pdo_update($this->table_users, array('weid' => '200') , array('rid' => $rid, 'from_user' => $from_user));
					}
				}
