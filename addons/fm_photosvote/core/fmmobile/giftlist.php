	...
	if ($_GPC['foo'] == 'giftshop') {
		...
		if ($_GPC['gifttype'] == 'yishiyong') {
			...
			foreach ($data as $key => $value) {
				...
				$data[$key]['time'] = '<div class="mui-input-row" style="height:40px;"><label>赠送时间</label><input type="text" value="'.date('Y-m-d H:i:s', $value['lasttime']).'" readonly="readonly"></div>';
			...
			$data = pdo_fetchall("SELECT * FROM " . tablename($this->table_user_zsgift) . ' WHERE rid = :rid AND tfrom_user = :tfrom_user ' . $uni . ' ORDER BY status ASC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, array(':rid' => $rid, ':tfrom_user' => $from_user));
			...
			foreach ($data as $key => $value) {
				...
				//$data[$key]['piaoshu'] = $g['piaoshu'];
				//$data[$key]['jifen'] = $g['jifen'];
				$data[$key]['time'] = '<div class="mui-input-row" style="height:40px;"><label>获得时间</label><input type="text" value="'.date('Y-m-d H:i:s', $value['createtime']).'" readonly="readonly"></div>';
				$data[$key]['tuser'] = '<div class="mui-input-row" style="height:40px;"><label>赠送者：</label><img class="ysimg" src="'.$this->getname($rid, $value['from_user'],'20', 'avatar').'"><span class="ystext">'.$this->getname($rid, $value['from_user']).'</span></div>';
			}
		}
