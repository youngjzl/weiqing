...
if (empty($_GPC['paymore'])) {
	if ($rbasic['ismiaoxian'] && $rbasic['mxnexttime'] != 0) {
		if (!isset($_COOKIE["fm_miaoxian"])) {
			setcookie("fm_miaoxian", 'startmiaoxian', time() + $rbasic['mxnexttime']);
...
if (!empty($gtfrom_user)) {
	...
	$videou = pdo_fetch("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid and  status = :status ORDER BY id ASC LIMIT 1", array(':from_user' => $gtfrom_user,':rid' => $rid,':status' => 1));//显示所有视频
	...
	if (!empty($item_tfrom)) {
		...
		if ($item_tfrom['vedio'] || $item_tfrom['youkuurl'] || $videou['videopath']) {
			if ($videou['videopath']) {
				$rightmedia_v = '<video id="videocon" controls width="100%" height="200" poster="' . $photos_tfrom . '" webkit-playsinline><source src="' . tomedia($videou['videopath']) . '" type="video/mp4" /><p class="vjs-no-js">你的浏览器不支持该视频</a></p></video>';
			}elseif ($item_tfrom['vedio']) {
...
if ($_GPC['gettype'] == 'getismp3') {
	$item_tfrom = pdo_fetch("SELECT * FROM " . tablename($this -> table_users) . " WHERE rid = :rid AND uid = :uid LIMIT 1", array(':rid' => $rid, ':uid' => $_GPC['uid']));
	$videou = pdo_fetch("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid and  status = :status ORDER BY id ASC LIMIT 1", array(':from_user' => $item_tfrom['from_user'],':rid' => $rid,':status' => 1));//显示所有视频
	if (empty($item_tfrom['vedio']) && empty($item_tfrom['youkuurl']) && empty($videou['videopath'])) {
		$fmdata = array(
			"success" => 1,
			"flag" => 1
		);
...
if (!empty($from_user)) {
	...
	$videoum = pdo_fetch("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid and  status = :status ORDER BY id ASC LIMIT 1", array(':from_user' => $from_user,':rid' => $rid,':status' => 1));//显示所有视频
