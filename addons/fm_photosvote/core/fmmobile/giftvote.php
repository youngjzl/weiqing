...
if ($op == 'phb') {
	...
	$datas = pdo_fetchall("SELECT from_user, giftnum FROM " . tablename($this->table_users) . ' WHERE rid = :rid AND giftnum > 0 ORDER BY giftnum DESC LIMIT 1, 1000000', array(':rid' => $rid));

	foreach ($datas as $key => $value) {
		$pmid = $key+2;
		$datas[$key]['pmid'] = $pmid;
		$datas[$key]['mlz'] = '魅力值 <span style="color:#f77090;">' . $value['giftnum'] . '</span>';
		$datas[$key]['avatar'] = '<img class="ysimg pull-left" src="'.$this->getname($rid, $value['from_user'],'20', 'avatar').'" width="30" height="30" style="border-radius: 30px;display: inherit;    border-radius: 30px;margin-right: 10px;">';
		$datas[$key]['username'] = $this->getname($rid, $value['from_user'],'10');
