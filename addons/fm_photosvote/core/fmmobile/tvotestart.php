...
//					$msg = '您每天最多可以投 '.$rvote['uni_daytpxz'].' 次，您当天的次数已经投完，请明天再来';
			...
			if ($_GPC['votepay'] == 1 && $_GPC['paystatus'] != 'success') {
				...
				$shuapiao = pdo_fetch("SELECT from_user , ip, ua FROM ".tablename($this->table_shuapiao)." WHERE (from_user = :from_user OR ip = :ip OR ua = :ua) and rid = :rid LIMIT 1", array(':from_user' => $from_user,':ip' => getip(), ':ua' => $mobileinfo['1'],':rid' => $rid));
				...
				if($from_user == $shuapiao['from_user'] || $shuapiao['ip'] == getip() || $shuapiao['ua'] == $mobileinfo['1']) {
					...
					$msg = empty($res) ? $msg : $res ;
					//$user = pdo_fetch("SELECT * FROM ".tablename($this->table_users)." WHERE from_user = :from_user and rid = :rid", array(':from_user' => $tfrom_user,':rid' => $rid));
					$nowuser = pdo_fetch("SELECT photosnum, xnphotosnum,hits FROM ".tablename($this->table_users)." WHERE from_user = :from_user and rid = :rid", array(':from_user' => $tfrom_user,':rid' => $rid));
					//增加积分
					$this->addjifen($rid, $from_user, $tfrom_user,array($nickname,$avatar,$sex,$msg),array($uniacid, $vote_times, $rdisplay['ljtp_total'],$nowuser['photosnum'],$nowuser['hits'],$rdisplay['cyrs_total']));
					if ($rvote['isanswer'] == 1) {
						if ($voteer['chance'] > 0) {
