...
defined('IN_IA') or exit('Access Denied');
		...
		foreach ($reply as $mid => $replys) {
			$reply[$mid]['num'] = pdo_fetchcolumn("SELECT COUNT(1) FROM ".tablename($this->table_users)." WHERE rid = :rid", array(':rid' => $replys['rid']));
			$reply[$mid]['is'] = pdo_fetchcolumn("SELECT COUNT(1) FROM ".tablename($this->table_users)." WHERE rid = :rid and from_user = :from_user", array(':rid' => $replys['rid'], ':from_user' => $from_user));
		...
		//查询参与情况
		$usernum = pdo_fetchcolumn("SELECT COUNT(1) FROM ".tablename($this->table_users)." WHERE from_user = :from_user", array(':from_user' => $from_user));
