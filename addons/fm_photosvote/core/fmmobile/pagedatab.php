...
defined('IN_IA') or exit('Access Denied');
		...
		foreach ($userlist as $key => $row) {
			...
			if (!empty($mphotosnum)) {
				...
				$gq = '<div style="width: 100%;height: 30px;position: absolute;z-index:100000000;"><a href="'.$purl.'" ><span class="rightmedia_pic"><strong class="bnum">查看高清图 '.$mphotosnum.' 张</strong></span></a></div>';
			}
			$videou = pdo_fetch("SELECT * FROM ".tablename($this->table_users_videoarr)." WHERE from_user = :from_user and rid = :rid and  status = :status ORDER BY id ASC LIMIT 1", array(':from_user' => $row['from_user'],':rid' => $rid,':status' => 1));
			if ($row['vedio'] || $row['youkuurl'] || $videou['videopath']) {
				$rightmedia = '<span class="rightmedia"><img src="'.FM_STATIC_MOBILE.'public/images/video.png?v=3.0" style="width:20px;"></span>';
				if ($videou['videopath']) {
					$userlist[$key]['rightmedia_v'] = $gq . '<video id="videocon" controls width="100%" height="200" poster="'.$userlist[$key]['avatar'].'" webkit-playsinline><source src="'.tomedia($videou['videopath']).'" type="video/mp4" /><p class="vjs-no-js">你的浏览器不支持该视频</a></p></video>';
				}elseif ($row['vedio']) {
