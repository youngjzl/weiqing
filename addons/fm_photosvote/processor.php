					...
					if ($rbasic['status']==0){
						...
						if ($tag == '1431' || ($this->message['event'] == 'SCAN' && !$rets)) {
							...
							return $this->respNews(array(
								...
								'PicUrl' => $picture,
								'Url' => $this->createMobileUrl('photosvote', array('rid' => $rid,'from_user'=>$from_user,'gfrom_user'=>$from_user)),
							));
						}
										...
										if (empty($_SESSION['bmstart'])) {
											...
											if (!empty($isuser)) {
												...
												return $this->respNews(array(
													...
													'Url' => $this->createMobileUrl('tuser', array('rid' => $rid, 'tfrom_user' => $from_user,'gfrom_user'=>$from_user))
												));
											}
...
//			$message = '在此活动期间，你总共可以投 '.$rvote['fansmostvote'].' 票，目前你已经投完！';
            $message = '投票成功，活动期间每天都可为你pick的选手投票哦~';
            return $message;
		}
