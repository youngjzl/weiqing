...
$(function() {
	...
	if($(".imgbox").length === 0) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				$("#next_page a").attr("hehe", nextHref);
				$("#page_nav").remove();
				$container.after(newNav);

				console.log(hackfour);
	...
	$(window).bind("scroll", function() {
		if($(".imgbox").length === 0 && parseInt($("#userc").html()) > 0) {
			location.reload();
		} else if($(document).scrollTop() + $(window).height() > $(document).height() - 10) {
			if($("body").attr("data-position") >= 0 && $("body").attr("data-position") != undefined) {
				return false;
			}
