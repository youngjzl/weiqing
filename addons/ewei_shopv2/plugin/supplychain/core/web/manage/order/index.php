...
class Index_EweiShopV2Page extends SupplychainWebPage {
    ...
    protected  function orderData($status, $st){
        ...
        $supplychain_user = $_W['supplychain_user'];
        ...
        $condition = " o.uniacid = :uniacid and o.supplychainid = :supplychainid and o.deleted=0 and o.isparent=0";
        $uniacid = $_W['uniacid'];
        $supplychainid = $_W['supplychainid'];

        $paras = $paras1 = array(':uniacid' => $uniacid, ':supplychainid' => $supplychainid);
        ...
        $sql = "select distinct o.* ,a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea,a.address as aaddress,a.street as astreet,d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,r.rtype,r.status as rstatus from " . tablename('ewei_shop_order') . " o"
                            ...
                            if (!empty($verifyinfo)) {
                                foreach ($verifyinfo as $k => $v) {
                                    ...
                                    if (!empty($verifyopenid)) {
                                        $verify_member = com('verify')->getSalerInfo($verifyopenid, $row['supplychainid']);
                                        $row['salerinfo'] .= "[" . $verify_member['salerid'] . "]" . $verify_member['salername'] . "(" . $verify_member['salernickname'] . ")";
                                        $verify_store = com('verify')->getStoreInfo($verifystoreid, $row['supplychainid']);
        ...
        if ($searchfield == 'member') {
            ...
            if (!empty($openidArr)) {
                ...
                $orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice FROM '.tablename('ewei_shop_order').' WHERE uniacid = '.$_W['uniacid'].' AND supplychainid = '.$supplychainid.' AND deleted=0 AND isparent=0 AND openid IN ('.$inOpenid.')'.$priceStatus);
            }else{
            ...
            $orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM '.tablename('ewei_shop_order').
                ' o left join '.tablename('ewei_shop_member_address').' a on o.addressid = a.id WHERE o.uniacid = '.$_W['uniacid'].'  AND o.deleted=0 AND o.isparent=0 AND o.supplychainid = '.$supplychainid.$priceCondition.$statuscondition);
            $totalmoney = $orderPrice['sumprice'];
            $total = $orderPrice['count'];
            ...
            $t = pdo_fetch(
                ...
                . " left join " . tablename('ewei_shop_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('ewei_shop_verifyorder_log') . " vol on vol.orderid=o.id "
//                . " left join " . tablename('ewei_shop_merch_saler') . " s on (s.openid = o.verifyopenid or s.id=vol.salerid) and s.uniacid=o.uniacid and s.supplychainid=o.supplychainid"
//                . " left join " . tablename('ewei_shop_member') . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid"
                . " left join " . tablename('ewei_shop_member') . " sm on sm.openid = o.openid"
...
//        $stores = pdo_fetchall('select id,storename from ' . tablename('ewei_shop_merch_store') . ' where uniacid=:uniacid and supplychainid = :supplychainid', array(':uniacid' => $uniacid, ':supplychainid' => $_W['supplychainid']));
