...
$params1 = array(':uniacid' => $_W['uniacid'], ':supplychainid' => $_W['supplychainid']);
$not_add = 0;
$maxgoods = intval($supplychain_user['maxgoods']);
if ($maxgoods > 0) {
...
$item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_goods') . " WHERE id = :id and uniacid=:uniacid and supplychainid=:supplychainid", array(':id' => $id, ':uniacid' => $_W['uniacid'], ':supplychainid' => $_W['supplychainid']));
...
$category = p('supplychain')->getCats();
...
//添加商品
if ($_W['ispost']) {
    if ($_W['supplychain_user']['can_edit'] != 1 && $item['import_id'] > 0)
    {
    ...
    $data = array(
        ...
        'supplychainid' => intval($_W['supplychainid']),
        'supplychaindisplayorder' => intval($_GPC['displayorder']),
    ...
    if (empty($_W['supplychain_user']['goodschecked'])) {
        //需要审核,只要修改过数据，就重新审核
        if ($needcheck) {
            $data['checked'] = 1;
        }
    ...
    if (empty($id)) {
        if (empty($_W['supplychain_user']['goodschecked'])) $data['checked'] = 1;
    ...
    show_json(1, array('url' => supplychainUrl('goods/edit', array('id' => $id, 'tab' => str_replace("#tab_", "", $_GPC['tab'])))));
}
//查询商品
if (!empty($id)) {
