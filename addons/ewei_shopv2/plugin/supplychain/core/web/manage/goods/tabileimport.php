...
class Tabileimport_EweiShopV2Page extends SupplychainWebPage
{
    public function main()
    {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($rows as $rownu => $col) {
                ...
                $item['supplychainid']=$_W['supplychainid'];
            ...
            setcookie("taobaoCSV",serialize($items));
//            session_start();
...
//        session_start();$items = $_SESSION['taobaoCSV'];
        $items = unserialize($_COOKIE['taobaoCSV']);
        if (empty($items)){
            return '';
        }
        $ret = $this->model->save_taobaocsv_goods($items[$num],$_W['supplychainid']);
        plog('tabileimport.main', '淘宝CSV宝贝批量导入' . $ret[goodsid]);

        if ($totalnum <= $num + 1) {
            unset($_COOKIE['taobaoCSV']);
        }
