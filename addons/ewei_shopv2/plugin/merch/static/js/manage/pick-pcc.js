...
var url=$('input[name=goodscatsurl]').val()
var areaJson= '';
$.get(url, function(result){
    areaJson=JSON.parse(result)
    areaJson=areaJson.result.data;
});
(function (execute) {
    ...
    Plugin.prototype = {
        ...
        render:function(){
            ...
            if(format && format!=""){
                ...
                contentStr = '<div class="pick-show">'+
                    ...
                    '<span class="pick-province">一级分类</span><i>/</i>'+
                    '<span class="pick-city">二级分类</span><i>/</i>'+
                    '<span class="pick-county">三级分类</span>'+
            ...
            if(setFormat){
                ...
                if(setFormat.match("province")){
                    if(slash==0){
                        strSpan = '<span class="pick-province">一级</span>';
                        this.format = "p";
                    } else if(slash==1){
                        strSpan = '<span class="pick-province">一级</span>'+
                            '<i>/</i>'+
                            '<span class="pick-city">二级</span>';
                        this.format = "p/c";
                    } else if(slash==2){
                        strSpan = '<span class="pick-province">一级</span>'+
                            '<i>/</i>'+
                            '<span class="pick-city">二级</span>'+
                            '<i>/</i>'+
                            '<span class="pick-county">三级</span>';
                        this.format = "p/c/c";
                    }
                    contentStr = '<div class="pick-show">'+
            ...
            if(this.config.width){
                ...
                this.ele.find(".pick-show span").css({"max-width":(parseInt(this.config.width) - parseInt(this.config.paddingLeft) - parseInt(this.config.arrowRight) - 16 - this.ele.find("i").outerWidth()*(checkNum-1) - 12 * checkNum -10 )/checkNum+9})
            }
            ...
            this.ele.find(".pick-city").click(function(e){
                var e = e || event;
                e.stopPropagation();
                if(_this.ele.find(".pick-province").html()!="请选择一级分类") {
                    $(this).parents("body").find(".pick-area span").removeClass("pressActive").css("background", "#fff");
                    $(this).parents("body").find(".pick-list").hide();
                    $(this).addClass("pressActive").css("background", _this.config.hoverColor).siblings("span").removeClass("pressActive").css("background", "#fff");
                    _this.createCity(e);
            ...
            this.ele.find(".pick-county").click(function(e){
                var e = e || event;
                e.stopPropagation();
                if(_this.ele.find(".pick-city").html()!="请选择二级分类"){
                    $(this).parents("body").find(".pick-area span").removeClass("pressActive").css("background","#fff");
                    $(this).parents("body").find(".pick-list").hide();
                    $(this).addClass("pressActive").css("background",_this.config.hoverColor).siblings("span").removeClass("pressActive").css("background","#fff");
        ...
        createProvince:function(e){
            ...
            for(var i=0;i<area.length;i++){
                if(i==0){
                    proStr = '<li class="ulli" data-code="">请选择一级分类</li>'
                }
        ...
        createCity:function(e){
            ...
            if(cityJson.length!=0){
                for(var j=0;j<cityJson.length;j++){
                    if(j==0){
                        cityStr = '<li class="ulli" data-code="">请选择二级分类</li>'
                    }
        ...
        createCounty:function(e){
            ...
            if(setCity=="请选择二级分类"){
                ...
                for(var t=0;t<countyJson.length;t++){
                    if(t==0){
                        countyStr = '<li class="ulli" data-code="">请选择三级分类</li>'
                    }
        ...
        setVal:function(_thisP,aim,s){
            ...
            if(s=="pro"){
                ...
                if($(aim).html()=="请选择一级分类"){
                    ...
                    this.ele.find(".pick-city").html("请选择二级分类");
                    this.ele.find(".pick-county").html("请选择三级分类");
                    ...
                    if(($(aim).html()=="台湾省" || $(aim).html()=="香港特别行政区" || $(aim).html()=="澳门特别行政区") && this.format!="p"){
                        ...
                        $(".pick-area-hidden").val($(aim).attr("data-code"));
                        return;
                    } else {
                ...
                if($(aim).html()=="请选择二级分类"){
                    this.ele.find(".pick-list").hide();
                    this.ele.find(".pick-county").html("请选择二级分类");
                    ...
                    var pro_areaCode = $("input.pick-area").attr("data-areacode").split(",")[0] + ",";
                    ...
                    $(".pick-list").scrollTop(0);
                    $(".pick-area-hidden").val(pro_areaCode+$(aim).attr("data-code"));
                    this.setBack(this);
                }
            } else if(s=="county"){               
                if($(aim).html()=="请选择三级分类"){
                    if(this.ele.find(".pick-county").html()=="请选择三级分类"){
                        //alert(1)
                    } else {
                        //alert(2)
                        this.ele.find(".pressActive").html($(aim).html());
                    ...
                    this.ele.find(".pressActive").html($(aim).html());
                    var pro_city_areaCode = $("input.pick-area").attr("data-areacode").split(",")[0] + ","+$("input.pick-area").attr("data-areacode").split(",")[1]+',';
                    ...
                    $(".pick-list").scrollTop(0);
                    $(".pick-area-hidden").val(pro_city_areaCode+$(aim).attr("data-code"));
                    this.setBack(this);
                }
            }
        },
