...
class Creditlog_EweiShopV2Page extends PluginMobileLoginPage {
	...
	function getlist(){
        ...
        $list = array();
        if (!empty($total)) {
            $sql = 'SELECT log.id,log.goodsid,g.title,g.thumb,g.credit,g.type,g.money,log.createtime,log.time_finish, log.status, g.thumb FROM ' . tablename('ewei_shop_creditshop_log') . ' log '
