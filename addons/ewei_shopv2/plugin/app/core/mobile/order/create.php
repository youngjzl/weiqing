...
//                }
            ...
            if (!empty($saleset)) {
                ...
                if (!empty($saleset['creditdeduct'])) {
                    ...
                    //如果抵扣的钱(15) 大于 商品的实际价格(5)减去秒杀的价格
                    ...
                    //$pure_price 等于 实际支付价格减去运费    实际支付价格为5
                    $pure_price = $realprice - $dispatch_price;  //-10
                    if ($pure_price < 0){
                        $pure_price = 0;
                    }
                    //如果  运费大于0  并且 每个积分可以抵扣的钱乘 是否开启不等于0  并且 抵扣的钱除  每个可以抵扣的钱 乘 是否开启抵扣 大于 实际价格减去运费的价格
                    if($dispatch_price > 0 && ($pmoney * $pcredit != 0) && ceil($deductmoney / $pmoney * $pcredit)>$pure_price){
                        //抵扣的钱等于   抵扣的钱(5) 减去 运费（15） = 0
//                        $deductmoney = $deductmoney - $pure_price;
                        //TODO #16958 待定
                        $deductmoney = $pure_price;
                    }
...
//        }
    ...
    function caculatecard($cardid,$dispatch_price,$totalprice,$discountprice=0,$isdiscountprice =0, $goods = [], $goods_dispatch = []){
        ...
        $return_array['totalprice'] = $totalprice;
        //根据会员卡规则计算后的会员折扣
        $return_array['discountprice'] = $discountprice; //普通会员优惠
        //根据会员卡规则计算后的促销折扣
        $return_array['isdiscountprice'] = $isdiscountprice;
        //选中的会员卡名字
        $return_array['cardname'] = $card['name'];
