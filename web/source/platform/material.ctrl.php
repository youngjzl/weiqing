...
if ($do == 'display') {
	...
	$cats= trim($_GPC['cats']);
    $cats = in_array($cats, array('hfgh', 'czgl', 'hwzc')) ? $cats : null;
	...
	if ($type == 'news') {
		$material_news_list = material_news_list($server, $search, array('page_index' => $page_index, 'page_size' => $page_size),$cats);
	} else {
		if (empty($server)) {
